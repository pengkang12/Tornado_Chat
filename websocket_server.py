...
#test merge request, create dev branch
...
class WebSocketHandler(tornado.websocket.WebSocketHandler):
            ...
            if target_key.split(':')[0] == 'user':
                payload = {
                    ...
                    "msg": {
                        ...
                        "class_type": obj['class_type'] if 'class_type' in obj else 'None',
                    },
                    "from": user_profile,
                ...
                if self.r.get(target_key + ":online") == "True":
                    ...
                    self.r.rpush(target_key + ":pipe", json_content)
                # this is a synchronization, we should use async.
                yield self.write_to_elastic_search(doc=payload)
                ...
                payload = {
                    ...
                    "msg": {
                        ...
                        "id": str(uuid.uuid1()),
                        "timestamp": now().strftime(settings.REST_FRAMEWORK['DATETIME_FORMAT']),
                        "at_all": obj['at_all'] if 'at_all' in obj else 'false',
                        "at": obj['at'] if 'at' in obj else [],
                    },
                    "room": room_id,
                    "from": user_profile
                }
    ...
    # write msg to ElasticSearch
    # @gen.coroutine
    def write_to_elastic_search(self, doc):
        # es = Elasticsearch(hosts=[settings.ELASTICSEARCH_URL])
        if doc['from']:
            body = {
                "msg_type": doc['msg_type'],
                "msg": doc['msg'],
                "id": doc['from']['id'] or 'none',
                "name": doc['from']['name'] or 'none',
                "mobile": doc['from']['mobile'] or 'none',
                "gaiid": doc['from']['gaiid'] or 'none',
                "uid": doc['from']['uid'] or 'none',
            }
        else:
            body = {
                "msg_type": doc['msg_type'],
        ...
        # res = es.index(index='websocket-msg', doc_type='msg', id=doc['msg']['id'], body=body)
        # LOG.info(res['created'])
        # LOG.info('end write to elastic search')
